...
static void des_key3(lua_State* ls, DES_key_schedule ks[3])
  ...
  string key(s, l);
  //K1补齐
  while(key.size() < 1 * sizeof(DES_cblock)) key.push_back('\0');
  //如果K2不存在，则K2 == K1
  if(key.size() == 1 * sizeof(DES_cblock))
    {
...
static int des_ecb(lua_State* ls, const int enc, const bool padding)
  ...
  if(padding && enc == DES_ENCRYPT)
    {
    pkcs7padding(data, sizeof(DES_cblock), true);
    }
  else
    {
    data.resize(data.size() - (data.size() % sizeof(DES_cblock)));
    }
  ...
  if(padding && enc == DES_DECRYPT)
    {
    pkcs7padding(data, sizeof(DES_cblock), false);
    }
