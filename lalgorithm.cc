...
static int LUA_C_md5(lua_State* ls)
  ...
  lua_pushlstring(ls, (const char*)&mv, sizeof(mv));
  return 1;
  }
//////////////////////////////////////////////////////////////////////////
...
static int LUA_C_getvarint(lua_State* ls)
  ...
  if(is)
    {
    int64 v = 0;
    const auto res = getvarint(v, (const unsigned char*)s, l);
...
static int LUA_C_TeaEncrypt(lua_State* ls)
  ...
  const auto res(TeaEncrypt(s, l, TEA_KEY(ks, kl),
...
static int LUA_C_TeaDecrypt(lua_State* ls)
  ...
  const auto res(TeaDecrypt(s, l, TEA_KEY(ks, kl),
    luaL_checkinteger(ls, 3),
    luaL_checkinteger(ls, 4)
    ));
...
static int LUA_C_TeanEncipher(lua_State* ls)
  ...
  const auto res(TeanEncipher(luaL_checkstring(ls, 1), TEA_KEY(ks, kl)));
...
static int LUA_C_TeanDecipher(lua_State* ls)
  ...
  const auto res(TeanDecipher(luaL_checkstring(ls, 1), TEA_KEY(ks, kl)));
